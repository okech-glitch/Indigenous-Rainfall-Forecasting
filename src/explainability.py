...
def run_shap(
    train_path: str, model_path: str, output_dir: str, num_samples: int = 512
) -> None:
    ...
    try:
        ...
        if isinstance(sv, list):
            ...
            if getattr(sv, "values", None) is not None and sv.values.ndim == 3:
                ...
                for i in range(num_classes):
                    ...
                    plt.tight_layout()
                    plt.savefig(
                        os.path.join(output_dir, f"shap_beeswarm_class_{i}.png"),
                        dpi=200,
                    )
                    plt.close()
            else:
                ...
                plt.tight_layout()
                plt.savefig(
                    os.path.join(output_dir, "shap_beeswarm.png"), dpi=200
                )
                plt.close()
        ...
        background = shap.sample(
            pd.DataFrame(Xt_dense), min(200, Xt_dense.shape[0])
        ).values
        kexpl = shap.KernelExplainer(
            lambda z: clf.predict_proba(z), background
        )
        ...
        if isinstance(sv, list):
            for i, sv_i in enumerate(sv):
                ...
                plt.tight_layout()
                plt.savefig(
                    os.path.join(output_dir, f"shap_summary_kernel_class_{i}.png"),
                    dpi=200,
                )
                plt.close()
        else:
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(output_dir, "shap_summary_kernel.png"), dpi=200
            )
            plt.close()


def run_lime(
    train_path: str, model_path: str, output_dir: str, num_samples: int = 5
) -> None:
    os.makedirs(output_dir, exist_ok=True)
    df = pd.read_csv(train_path)
    artifact = joblib.load(model_path)
    ...
    for i in range(min(num_samples, len(X))):
        ...
        x_inst_dense = (
            x_inst.toarray()[0] if hasattr(x_inst, "toarray") else x_inst[0]
        )
