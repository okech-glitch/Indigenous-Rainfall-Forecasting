...
def test_train_and_predict_tmp(tmp_path):
    ...
    df_train = pd.DataFrame({
        ...
        "cloud": ["LOW", "HIGH", "MED"] * 6 + ["LOW", "HIGH"],
        "wind": ["CALM", "BREEZY"] * 10,
        "rain_mm": [0.0, 12.3, 3.2, 0.5] * 5,
        "Target": ["NORAIN", "SMALL", "SMALL", "NORAIN"] * 5,
    })
    ...
    pd.DataFrame({
        "ID": ["X", "Y"],
        "Target": ["NORAIN", "NORAIN"]
    }).to_csv(sample_sub_csv, index=False)
    ...
    predict_and_submit(
        str(test_csv), str(model_path), str(sample_sub_csv), str(submission_path)
    )
